...
fn unit_propigation() {
    ...
    let cnf = CNF::new(Arc::clone(&symbol_table), vec![vec![-3, 1, 2, 4], vec![-2], vec![3]]);
...
fn dpll() {
    ...
    let cnf = CNF::new(Arc::clone(&symbol_table), vec![vec![1], vec![-1]]);
    assert!(!cnf.is_sat());

    // Argument x -> y, x, therefore y
    let cnf = CNF::new(Arc::clone(&symbol_table), vec![vec![-1, 2], vec![1], vec![-2]]);
    assert!(!cnf.is_sat());

    // Invalid argument x -> y, y, therefore x
    let cnf = CNF::new(Arc::clone(&symbol_table), vec![vec![-1, 2], vec![2], vec![-1]]);
    assert!(cnf.is_sat());
}
